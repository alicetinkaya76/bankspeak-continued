#!/usr/bin/env bash
# Round-13 repaired reproducer (C38). Based on the round-13 auditor's
# corrected script (archived verbatim as
# reproduce_round12_blockers_round13_audit.sh) with one portability shim:
# sha256 hashing falls back to `shasum -a 256` where GNU sha256sum is
# absent (macOS). Expected nonzero ABORT exits are captured in subshells;
# stdout and stderr are kept separate; the four verdict lines
# (A1/A2 NOT REPRODUCED, 2x B ... REJECTED) always print.
set -uo pipefail
...
( cd "$REPO"; "$PY" "${COMMON[@]}" >"$TMP/no_hash.out" 2>"$TMP/no_hash.err" ); R_NO=$?
( cd "$REPO"; "$PY" "${COMMON[@]}" --calib-expected-sha256 "$CAL_SHA" >"$TMP/hash.out" 2>"$TMP/hash.err" ); R_HASH=$?
...
PY
BROKEN_SHA="$(sha256 "$BROKEN")"
( cd "$REPO"; "$PY" "$REPO/src/mde_sim.py" --mode curve --family p1p2 --theta-grid 0.0:0.0:1.0 --reps 2 --B 19 --sigma-delta 0.1 --calib-json "$BROKEN" --calib-expected-sha256 "$BROKEN_SHA" >"$TMP/broken.out" 2>"$TMP/broken.err" ); R_BROKEN=$?
...
CFG = {'api': {'base_url': 'http://fixture', 'rows_per_page': 5,
               'format': 'json', 'lang_exact': 'English', 'fields': ['id'],
               'max_retries': 1, 'timeout': 1, 'backoff_base': 0,
               'sleep_seconds': 0}}
base = {'docty': 'X', 'count': '5', 'display_title': 'A',
        'docdt': '2024-01-01T00:00:00Z', 'repnb': 'R1', 'volnb': '1'}
class Response:
    status_code = 200
    def __init__(self, docs):
        self.payload = {'total': len(docs),
                        'documents': {f'D{i}': d for i, d in enumerate(docs)}}
