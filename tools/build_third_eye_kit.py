...
INCLUDE = [
    ...
    # Named in the manuscript or the supplement, and absent from the kit until
    # an external review listed them one by one. A reader told "Table 5c is
    # regenerated by tools/block_origin_enumeration.py" who cannot find that
    # file in the kit has been handed a claim, not a check.
    ("tools/block_origin_enumeration.py", "07_code",
     "Table 5c: exact PASS-P p at each block origin, time order kept; also "
     "reports the rotated values an earlier version published"),
    ("tools/check_submission_metadata.py", "07_code",
     "refuses if title, abstract length, page counts, test count or the "
     "archived version disagree across the submission documents"),
    ("tools/placeholder_report.py", "07_code",
     "refuses to build while an author bracket is unfilled"),
    ("tools/package_evidence_deposit.py", "07_code",
     "builds the Zenodo evidence deposit and its metadata"),
    ("tools/build_public_repo.py", "07_code",
     "builds the public code mirror; refuses on IMF content or on a "
     "contradiction between its own include and deny lists"),
    ("tools/build_third_eye_kit.py", "07_code",
     "builds this kit; refuses if a staged file's hash disagrees with the repo"),
